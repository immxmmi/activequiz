...
async function buildPdf(title,question, answers, rightAnswer, labels, chartType) {
    ...
    const typ = "{type:'"+chartType+"',data:{labels:[2012  ,2013  ,2014,2015, 2016],datasets:[{label:'"+title+"',data:[120,60,50,180,120]}]}}";
...
async function createPdf(sessionID) {
    ...
    getQuizDataBySession(sessionID).then(async (quizData) => {
        ...
        getChartDataBySessionID(sessionID).then((data) => {
            ...
            buildPdf(title.toString(),'question', 'answers', 'rightAnswer', labels, 'pie');
